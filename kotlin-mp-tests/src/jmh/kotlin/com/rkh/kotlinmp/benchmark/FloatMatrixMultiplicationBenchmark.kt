...
open class FloatMatrixMultiplicationBenchmark {
    ...
    lateinit var matrixB: FloatArray
    lateinit var matrixC: FloatArray

    @Setup(Level.Trial)
    fun setup() {
        val totalElements = size * size
        // Using modulo to prevent integer overflow during the O(N^3) multiplication
        matrixA = FloatArray(totalElements) { it.toFloat() % 100 }
        matrixB = FloatArray(totalElements) { (it.toFloat() * 2) % 100 }
        matrixC = FloatArray(totalElements)
    }
