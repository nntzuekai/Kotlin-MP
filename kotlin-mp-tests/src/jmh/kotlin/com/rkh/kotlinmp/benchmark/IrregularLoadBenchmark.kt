...
@Fork(1)
open class IrregularLoadBenchmark {

    @Param("1000", "3000", "5000", "10000", "15000", "30000")
    var size: Int = 0

    lateinit var workUnits: IntArray
    lateinit var results: IntArray

    @Setup(Level.Trial)
    fun setup() {
        workUnits = IntArray(size) { index ->
            if (index < size / 8) 4096 else 32
        }
        results = IntArray(size)
    }

    private fun irregularKernel(index: Int) {
        val iterations = workUnits[index]
        var acc = index + 1
        repeat(iterations) {
            acc = (acc * 1664525 + 1013904223) and 0x7fffffff
        }
        results[index] = acc
    }
    ...
    fun kotlinMpDynamicChunked() {
        omp {
            parallelFor(0 until size, Schedule.Dynamic(8)) { i ->
                irregularKernel(i)
            }
        }
    }
